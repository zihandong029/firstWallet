...
if (window.simpleCryptoWalletInjected) {
  ...
  class EIP6963Provider {
      ...
      window.addEventListener("message", (event) => {
        ...
        if (pendingRequest) {
          ...
          if (error) {
            ...
            if (pendingRequest.method === "eth_requestAccounts" && result) {
              this.accounts = result
              this.selectedAddress = result[0] || null
              this.isConnected = true
              this.emit("connect", { chainId: this._chainId })
              this.emit("accountsChanged", result)
            } else if (pendingRequest.method === "eth_chainId" && result) {
              this.updateChainInfo(result, parseInt(result, 16).toString())
            } else if (pendingRequest.method === "net_version" && result) {
              this.updateChainInfo(`0x${parseInt(result).toString(16)}`, result)
            } else if (pendingRequest.method === "wallet_switchEthereumChain") {
              // Chain switch successful, request updated chain info
              setTimeout(() => {
                this.request({ method: "eth_chainId" }).catch(console.error)
              }, 100)
            }
    ...
    // Update chain info
    updateChainInfo(chainId, networkVersion) {
      const oldChainId = this._chainId
      this._chainId = chainId
      this._networkVersion = networkVersion

      // Emit chainChanged event if chain actually changed
      if (oldChainId && oldChainId !== chainId) {
        console.log(
          `🔄 [EIP-6963] Chain changed from ${oldChainId} to ${chainId}`
        )
...
function announceProvider() {
  ...
  const announceEvent = new CustomEvent("eip6963:announceProvider", {
    detail: Object.freeze({
      ...
      provider: walletProvider // 不要冻结 provider，保持可变
    })
  })
